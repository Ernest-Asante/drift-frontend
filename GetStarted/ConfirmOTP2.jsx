...
function ConfirmOTP2({route, navigation}){ 
    const [otp, setOtp] = useState(["", "", "", ""]); 
    const [timeOut, setTimeOut] = useState(false);
    const { contact, identity, firstName, lastName } = route.params;
  ...
  useEffect(() => { 
    ...
    const handleOTPResend = async () => {
     ...
     // console.log('Combined OTP:', combinedOtp);
      ...
      try {
        const response = await fetch('http://10.20.32.44:3001/otpresend2', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ identity: identity, contact: contact}),
        });
    ...
    const handleOTPSubmit = async () => {
      try {
        ...
        const response = await fetch('http://10.20.32.44:3001/mmverifyotp2', {
          method: 'POST',
          headers: {  
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ identity: identity, otp: combinedOtp, contact: contact, firstName: firstName, lastName: lastName}),
        }); 
     ...
     /*   if (!response.ok) {
      ...
      if (data.error === 'error fetching data') {
       ...
       // navigation.navigate('Payment', { contact });
        ...
        console.log('Invalid OTP') 
      }  else if(data.message === 'Error inserting data'){
        console.log('Error inserting data') 
      }  else if(data.message === 'Data inserted successfully'){ 
        console.log('Data inserted successfully') 
        navigation.navigate('PanicDetail', { identity });
      }  else if(data.message === 'Unexpected error during insert'){
        console.log('Unexpected error during insert') 
