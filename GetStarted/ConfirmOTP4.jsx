...
function ConfirmOTP4({route, navigation}){ 
  ...
  useEffect(() => { 
    ...
    const handleOTPSubmit = async () => {
      try {
        ...
        const response = await fetch('http://172.20.10.3:3001/mmverifyotp4', {
          method: 'POST',
          headers: {  
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ identity: identity, otp: combinedOtp, contact: contact, firstName: firstName, lastName: lastName, dataId}),
        }); 
     ...
     /*   if (!response.ok) {
      ...
      if (data.error === 'error fetching data') {
       ...
       // navigation.navigate('Payment', { contact });
        ...
        navigation.navigate('PanicDetail2', { contacts:contact });
