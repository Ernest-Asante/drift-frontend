...
function ConfirmOTP3({route, navigation}){ 
  ...
  useEffect(() => { 
    ...
    const handleOTPSubmit = async () => {
      try {
        ...
        const response = await fetch('http://172.20.10.7:3001/mmverifyotp3', {
          method: 'POST',
          headers: {  
            'Content-Type': 'application/json',
          },
     ...
     /*   if (!response.ok) {
      ...
      if (data.error === 'error fetching data') {
       ...
       // navigation.navigate('Payment', { contact });
        ...
        navigation.navigate('PanicDetail', { identity:identity });
